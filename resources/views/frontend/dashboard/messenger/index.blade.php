                                            ...
                                            <div class="wsus__chatlist_body">
                                                @foreach ($chatUsers as $user)
                                                    <button class="nav-link chat_user_profile"
                                                        data-id="{{ $user->receiver->id }}" data-bs-toggle="pill"
                                                        data-bs-target="#v-pills-home" type="button" role="tab"
                                                        aria-controls="v-pills-home" aria-selected="true">
                                                        @php
                                                            $unSeenMessages = App\Models\Chat::where('sender_id',$user->receiver->id)
                                                            ->where('receiver_id',auth()->user()->id)->where('status',0)->count();
                                                        @endphp
                                                        <div class="wsus_chat_list_img {{ $unSeenMessages > 0 ? 'msg-notification' : ''}}">
                                                            <img id="chat_user_image"
                                                                src="{{ asset($user->receiver->image) }}" alt="user"
                                                                class="img-fluid">
                                                            <span class="pending d-none" id="pending-6">0</span>
                                                        </div>
                                                        <div class="wsus_chat_list_text">
                                                            <h4 id="chat_user_name">{{ $user->receiver->name }}</h4>
                                                        </div>
                                                    </button>
                                                @endforeach
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $(document).on('click', '.chat_user_profile', function() {
                ...
                chatBox.attr('data-inbox', receiverID);
                chatBox.html("");
                        ...
                        $.each(response.chat, function(index, value) {
                            ...
                            `;
                                ...
                                <div class="wsus__chat_single">
                                    <div class="wsus__chat_single_img">
                                        <img src="${response.receiverImage}" alt="user" class="img-fluid">
                                    </div>
                                    <div class="wsus__chat_single_text">
                                        <p>${value.message}</p>
                                        <span>${formatDateTime(value.created_at)}</span>
